...
var mongooseEncryption = function(schema, options) {
  ...
  _.defaults(options, {
    ...
    decryptPostSave: false, // allow for skipping the decryption after save for improved performance
  ...
  var getKeyForQueryResponse = doc => {
    ...
    if (typeof key === "undefined") {
      throw new Error("Could not retrieve keyEncrypt from keyStore.");
    }
    // call function associated with model instance. TODO save in here instead?
    return key;
  };

  var setKeyFromQuery = query => {
    try {
  ...
  var getKeyFromDocument = doc => {
    let encryptionKey = doc._myKey;
    if(typeof encryptionKey === 'undefined'){
      throw new Error("Did not find required keyEncrypt in Mongoose document. ");
    }
    doc._myKey = null;
    return encryptionKey;
  };

  var keyIntoBuffer = key => {
    try {
      var encryptionKey = new Buffer(key, "base64");
    } catch (err) {
      throw new Error("checkKey: could not convert keyEncrypt into a buffer." + err);
    }
  ...
  if (options.middleware) {
    ...
    schema.pre("save", function(next) {
      ...
      // get Keys
      let encryptionKey = getKeyFromDocument(that);
