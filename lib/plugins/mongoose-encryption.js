...
*/
  ...
  if (options.fields) {
    ...
    console.warn(
      "options.fields has been deprecated. please use options.encryptedFields"
    );
  }
  if (options.exclude) {
    options.excludeFromEncryption = options.exclude;
    console.warn(
      "options.fields has been deprecated. please use options.excludeFromEncryption"
    );
  }
  ...
  let keyStoreGet = doc => {
    ...
    console.log("leyStoreGet" + doc);
    let id = doc[idForKey].toJSON();
    // call function associated with model instance. TODO save in here instead?
    return getKey(id);
  };

  let keyStoreSet = query => {
    let id = query._conditions[idForKey];
    let keys = {
      signingKey: query._conditions["keySign"],
      encryptionKey: query._conditions["keyEncrypt"]
    };
    setKey(id, keys);
  };
  ...
  var computeAC = function(doc, fields, version, modelName) {
    ...
    let signingKey = keyStoreGet(doc).signingKey;
  ...
  schema.methods.encrypt = function(cb) {
    ...
    crypto.randomBytes(IV_LENGTH, function(err, iv) {
      ...
      let encryptionKey = that._myKey.keyEncrypt;
      that._myKey = null;
      console.log("encrypt() with key : " + encryptionKey);
  ...
  schema.methods.decryptSync = function() {
    ...
    if (this._ct) {
      ...
      let encryptionKey = keyStoreGet(this).encryptionKey;
